...
def main():
    ap = argparse.ArgumentParser(description="HDBSCAN clustering for data/raw/export.csv text column using Sentence-BERT")
    ...
    ap.add_argument("--min-cluster-size", type=int, default=15, help="HDBSCAN minimum cluster size")
    ap.add_argument("--min-samples", type=int, default=5, help="HDBSCAN min_samples")
    ap.add_argument("--cluster-selection-epsilon", type=float, default=0.0, help="HDBSCAN cluster_selection_epsilon")
    ap.add_argument("--use-umap", action="store_true", help="Reduce embeddings with UMAP before HDBSCAN")
    ...
    ap.add_argument("--output-csv", default=str(DATA_PROCESSED_DIR / "export_hdbscan_clustered.csv"), help="Output CSV with cluster labels")
    ap.add_argument("--report", default=str(REPORTS_DIR / "hdbscan_cluster_report.txt"), help="Output report path")
    ...
    sil, dbi, metric_scope = compute_hdbscan_metrics(
        X_cluster,
        labels,
    ...
    ex = exemplar_texts(X_cluster, labels, cleaned2, per_cluster=args.examples)
    ...
    with open(args.report, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"min_cluster_size={args.min_cluster_size} min_samples={args.min_samples} cluster_selection_epsilon={args.cluster_selection_epsilon} metric=euclidean use_umap={args.use_umap} reducer={reducer_used}"
            + (
                f" umap_components={args.umap_components} umap_neighbors={args.umap_neighbors} umap_min_dist={args.umap_min_dist}"
                if args.use_umap
                else ""
            )
            + "\n"
